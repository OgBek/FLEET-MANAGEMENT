        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-4 py-5 border-b border-gray-200 sm:px-6 flex justify-between items-center">
                ...
                @if($notifications->whereNull('read_at')->count() > 0)
                    <form action="{{ route('maintenance.notifications.mark-all-as-read') }}" method="POST" class="inline">
                        @csrf
                ...
                <div class="flex items-start space-x-4">
                    ...
                    <div class="flex-shrink-0 flex items-center space-x-4">
                        @if(is_null($notification->read_at))
                            <form action="{{ route('maintenance.notifications.mark-as-read', $notification->id) }}" method="POST">
                                @csrf
                        ...
                        @if(isset($notification->data['link']))
                            <a href="{{ route('maintenance.notifications.show', $notification->id) }}" class="text-blue-600 hover:text-blue-800">
                                View details

        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg font-medium text-gray-900">Create New Vehicle Category</h3>
                ...
                <form action="{{ route('admin.vehicle-categories.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="priority_level" 
                            ...
                            <option value="1" {{ old('priority_level') == 1 ? 'selected' : '' }}>High Priority (VIP)</option>
                            <option value="2" {{ old('priority_level') == 2 ? 'selected' : '' }}>Medium Priority (Department)</option>
                            <option value="3" {{ old('priority_level') == 3 ? 'selected' : '' }}>Low Priority (General)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="vehicle_types[]" 
                            ...
                            @foreach($vehicleTypes as $type)
                                <option value="{{ $type->id }}" {{ in_array($type->id, old('vehicle_types', [])) ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
